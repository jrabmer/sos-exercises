...
class AttributeController(param.Parameterized):

    selected_attribute = param.ObjectSelector(default=None, objects=[], label='Select attribute')

    def __init__(self, object_options, calculate, **params):
        super(AttributeController, self).__init__(**params)
        self._calculate = calculate
        
        self.selected_attribute = object_options[0]  # Set the default to the first option
        self.param.selected_attribute.objects = object_options
    
    @param.depends("selected_attribute", watch=True)
    def _update_selected_object(self):
        self._calculate(self.selected_attribute)
